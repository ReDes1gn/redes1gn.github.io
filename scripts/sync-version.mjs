...
// bottom of index.html fetches /releases/latest and rewrites the download hrefs, the
// version label and the JSON-LD after load. That is enough for a human with a working
// network. It is not enough for anyone else.
//
// Everything that does not run JavaScript reads the STATIC file: search crawlers, the
// AI answer engines this site carries schema.org markup for in the first place, link
...
// not a fallback, it is a second claim. Nothing kept this file honest before, and it
// showed: the page claimed softwareVersion 1.6.6 while 1.6.7 was the release.
//
...
// no longer tracking anything), the page offers a build this script has no URL rule
// for, this script has a build the page no longer offers, or the release does not
// actually contain the asset a link resolves to (the release job renamed it). A
// download button that 404s looks exactly like a working site until somebody clicks
// it, so that case is a hard failure here rather than a silent one in a browser.
...
const DL_ASSETS = {
  'win-x64': 'redesign-windows-x64.exe',
  'win-x64-zip': 'redesign-windows-x64.zip',
  'win-x64-tray': 'redesign-windows-x64-with-tray.zip',
  'macos-arm64': 'redesign-macos-arm64.tar.gz',
  'linux-x64': 'redesign-linux-x64.tar.gz',
...
/** The tag as a bare `1.6.7`, plus the names of every asset the release actually
...
async function latestRelease() {
  const headers = { Accept: 'application/vnd.github+json', 'User-Agent': 'redesign-site-sync' };
